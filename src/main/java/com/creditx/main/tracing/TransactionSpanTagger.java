...
/**
 * Minimal helper to attach the business transactionId as a span tag so that Zipkin queries
 * (tagQuery: transactionId=123) can correlate asynchronous work across services without scattering
 * Tracer usage everywhere.
 */
@Component
...
public class TransactionSpanTagger {
  ...
  /**
   * Tag the current span with the provided transactionId (if a span exists). Safe no-op when there
   * is no active span or id is null.
   */
