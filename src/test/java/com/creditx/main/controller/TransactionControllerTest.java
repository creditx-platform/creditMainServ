...
class TransactionControllerTest {
  ...
  void createTransaction_success() throws Exception {
    given(transactionService.createInboundTransaction(any())).willReturn(
        CreateTransactionResponse.builder().transactionId(999L).status(TransactionStatus.PENDING)
            .build());
    ...
    mockMvc.perform(
            post("/api/transactions").contentType(MediaType.APPLICATION_JSON).content(requestBody))
        .andExpect(status().isAccepted()).andExpect(jsonPath("$.transactionId").value(999))
  ...
  void commitTransaction_success() throws Exception {
    given(transactionService.commitTransaction(eq(999L), any())).willReturn(
        CommitTransactionResponse.builder().transactionId(999L).status(TransactionStatus.SUCCESS)
            .message("Transaction committed successfully").build());
    ...
    mockMvc.perform(post("/api/transactions/999/commit").contentType(MediaType.APPLICATION_JSON)
  ...
  void commitTransaction_invalidPathVariable() throws Exception {
    ...
    mockMvc.perform(post("/api/transactions/invalid/commit").contentType(MediaType.APPLICATION_JSON)
        .content(requestBody)).andExpect(status().isBadRequest());
  }
  ...
  void createCashbackTransaction_success() throws Exception {
    given(transactionService.createCashbackTransaction(any())).willReturn(
        CreateTransactionResponse.builder().transactionId(555L).status(TransactionStatus.SUCCESS)
            .build());
    ...
    mockMvc.perform(post("/api/transactions/cashback").contentType(MediaType.APPLICATION_JSON)
